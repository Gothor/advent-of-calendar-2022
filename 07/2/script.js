...
class Directory extends Node {
  ...
  findDirectoriesWithSizeHigherThan(size) {
    const directories = [];
    ...
    if (this.size >= size) {
      ...
      for (const child of this.children) {
        ...
        directories.push(...child.findDirectoriesWithSizeHigherThan(size));
      }
    }
...
// Find smallest directory to delete to get enough space
const minSizeToFree = sizeRequired - (fileSystemSize - root.size);

const bigDirectories = root.findDirectoriesWithSizeHigherThan(minSizeToFree)
  .sort((a, b) => a.size < b.size ? -1 : 1);

console.log(bigDirectories[0].size);