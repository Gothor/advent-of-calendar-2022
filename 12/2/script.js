...
const lowestElevations = findAll(heightMap, 0);
const end = find(heightMap, 26);

explore(heightMap, distanceMap, end.x, end.y, 0, null);

console.log(Math.min(...lowestElevations.map(pos => distanceMap[pos.y][pos.x].distance)));
...
function explore(heightMap, distanceMap, x, y, distance, from) {
    ...
    for (const direction of directions) {
        ...
        if (
            ...
            && currentHeight <= heightMap[nextY][nextX] + 1
